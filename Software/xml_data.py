...
# Sử dụng str.maketrans() cho Python 3 để thay thế dấu [] bằng {}
trans = str.maketrans('[]', '{}')

def validate_args():
    if len(sys.argv) != 2:
        print(f"Invalid number of arguments ({len(sys.argv)}). Usage: python xml_data.py C:/Users/ADMIN/Downloads/New_folder/haarcascade_frontalface_default_int32.xml")
        sys.exit(-1)
...
def extract_stage_data(root):
    ...
    for stage in root.findall("./cascade/stages/"):
        stage_thresholds.append(float(stage.find('stageThreshold').text))
        ...
        for w_classifier in stage.findall("./weakClassifiers/_"):
            ...
            node_thresh = float(internal_nodes[3])
            l_val = float(leaf_values[0])
            r_val = float(leaf_values[1])
...
def extract_rectangles(root):
    ...
    for feature in root.findall("./cascade/features/"):
        ...
        for rect in feature.findall("./rects/"):
            # Chia tách các giá trị và chuyển đổi chúng
            rect_values = [float(val.translate(trans)) for val in rect.text.strip().split(' ')]
...
def main():
    ...
    c_file_name = xml_file_path.replace(".xml", ".c")
    ...
    write_to_c_file(c_file_name, f"\nconst float stageThresholds[{len(stage_thresholds)}] = {str(stage_thresholds).translate(trans)};\n")
    write_to_c_file(c_file_name, f"\nconst float nodes[{len(nodes)}] = {str(nodes).translate(trans)};\n")
