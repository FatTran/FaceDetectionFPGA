...
#pragma HLS INTERFACE mode=m_axi port=ii offset=slave bundle=gmem0 depth=HEIGHT*WIDTH
#pragma HLS INTERFACE mode=m_axi port=iiSq offset=slave bundle=gmem0 depth=HEIGHT*WIDTH
#pragma HLS INTERFACE mode=m_axi port=stageOrga offset=slave bundle=gmem1 depth=25
#pragma HLS INTERFACE mode=m_axi port=stageThresholds offset=slave bundle=gmem2 depth=25
#pragma HLS INTERFACE mode=m_axi port=stageNodes offset=slave bundle=gmem3 depth=8739
#pragma HLS INTERFACE mode=m_axi port=featOrga offset=slave bundle=gmem4 depth=2913
#pragma HLS INTERFACE mode=m_axi port=stageRects offset=slave bundle=gmem5 depth=31915

#pragma HLS INTERFACE mode=m_axi port=detectedX offset=slave bundle=gmem6 depth=100
#pragma HLS INTERFACE mode=m_axi port=detectedY offset=slave bundle=gmem7 depth=100
#pragma HLS INTERFACE mode=m_axi port=detectedW offset=slave bundle=gmem8 depth=100
#pragma HLS INTERFACE mode=m_axi port=detectedH offset=slave bundle=gmem9 depth=100
//
#pragma HLS INTERFACE mode=s_axilite port=rows
#pragma HLS INTERFACE mode=s_axilite port=cols
#pragma HLS INTERFACE mode=s_axilite port=factor
#pragma HLS INTERFACE mode=s_axilite port=return
	...
	for(int y = 0; y < rows - WIN_HEIGHT; y++){
		...
		for(int x = 0; x < cols - WIN_WIDTH; x++){
			...
			X1 = (y ) * cols + (x);
			X2 = (y ) * cols + (x + WIN_WIDTH);
			X3 = (y + WIN_HEIGHT) * cols + (x);
			X4 = (y + WIN_HEIGHT) * cols + (x + WIN_WIDTH);

			stddev = iiSq[X1] - iiSq[X2] - iiSq[X3] + iiSq[X4];

			mean = ii[X1] - ii[X2] - ii[X3] + ii[X4];
...
#pragma HLS UNROLL
			...
			if (result > 0) {
			    detectedX[detectCounter] = (ap_int<32>) x * factor;
			    detectedY[detectCounter] = (ap_int<32>) y * factor;
			    detectedW[detectCounter] = (ap_int<32>) 24 * factor;
			    detectedH[detectCounter] = (ap_int<32>) 24 * factor;
			    detectCounter++;
			}
		}
	}
